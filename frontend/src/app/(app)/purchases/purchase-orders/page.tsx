...
interface PurchaseOrder {
    ...
    number: string;
    date: string;
    status: 'draft' | 'issued' | 'partially_billed' | 'billed' | 'cancelled';
    total: number;
    party: {
        id: number;
        name: string;
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-slate-500">Track purchase indents and orders sent to vendors</p>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50 text-slate-500 text-sm border-b border-slate-200">
                                <th className="px-6 py-3 font-medium">PO Info</th>
                                ...
                                orders?.data?.map((order: PurchaseOrder) => (
                                    <tr key={order.id} className="hover:bg-slate-50 transition-colors group">
                                        <td className="px-6 py-4">
                                            <Link href={`/purchases/purchase-orders/${order.id}`} className="block">
