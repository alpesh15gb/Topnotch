...
export default function NewInvoicePage() {
    ...
    const [activeTenantState, setActiveTenantState] = useState('Maharashtra'); // Mocked for now, should come from tenant profile
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6 pb-24">
            <div className="flex items-center gap-4">
                <Link href="/sales/invoices" className="p-2 hover:bg-slate-200 rounded-full transition-colors text-slate-500">
                    <ArrowLeft size={20} />
                </Link>
                <div>
                    <h1 className="text-2xl font-bold text-slate-800">New Invoice</h1>
                    <p className="text-sm text-slate-500">Draft a new sales invoice</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col h-full">
                        <h3 className="text-sm font-medium text-slate-700 mb-2">Customer Notes</h3>
                ...
                <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 p-4 px-6 flex justify-end gap-3 z-30 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] pl-24 transition-all duration-300">
                    <Link href="/sales/invoices" className="px-6 py-2 border border-slate-300 text-slate-700 bg-white rounded-lg hover:bg-slate-50 font-medium transition-colors">
                        Cancel
                    </Link>
                    <button
                        type="submit"
