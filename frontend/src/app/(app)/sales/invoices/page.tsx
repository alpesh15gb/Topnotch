        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-slate-500">Create and manage your sales invoices</p>
                </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50 text-slate-500 text-sm border-b border-slate-200">
                                <th className="px-6 py-3 font-medium">Invoice Info</th>
                            ...
                            {isLoading ? (
                                ...
                                <tr>
                                    <td colSpan={5} className="px-6 py-12 text-center text-slate-500">
                                        <div className="flex flex-col items-center justify-center">
                                            ...
                                            <p className="text-sm mt-1">Try adjusting filters or create a new invoice to get started.</p>
                                            <Link
                                                href="/sales/invoices/new"
                                ...
                                invoices?.data?.map((invoice: Invoice) => (
                                    <tr key={invoice.id} className="hover:bg-slate-50 transition-colors group">
                                        <td className="px-6 py-4">
                                            <Link href={`/sales/invoices/${invoice.id}`} className="block">
                                        ...
                                        <td className="px-6 py-4">
                                            <Link href={`/masters/parties/${invoice.party.id}`} className="font-medium text-slate-700 hover:text-blue-600 transition-colors">
                                                {invoice.party.name}
                                            </Link>
                                        </td>
                                        <td className="px-6 py-4">
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            ...
                                            {invoice.balance > 0 && invoice.balance < invoice.total ? (
                                                <div className="text-xs text-amber-600 mt-0.5 font-medium">
                                                    Bal: ₹{invoice.balance.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                                                </div>
                                            ) : null}
                                        </td>
                ...
                <div className="p-4 border-t border-slate-200 flex items-center justify-between text-sm text-slate-500">
                    <div>Showing {invoices?.data?.length || 0} invoices</div>
