...
import { Plus, Search, Receipt, Download, Send, Handshake } from 'lucide-react';
...
export default function EstimatesPage() {
    ...
    const { data: estimates, isLoading } = useQuery({
        ...
        queryFn: async () => {
            const res = await api.get('/v1/estimates', {
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-slate-500">Create quotes and proposals for your customers</p>
                </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50 text-slate-500 text-sm border-b border-slate-200">
                                <th className="px-6 py-3 font-medium">Estimate Info</th>
                        ...
                        <tbody className="divide-y divide-slate-200">
                            {isLoading ? (
                                ...
                                <tr>
                                    <td colSpan={5} className="px-6 py-12 text-center text-slate-500">
                                        <div className="flex flex-col items-center justify-center">
                                            ...
                                            <p className="text-lg font-medium text-slate-700">No estimates found</p>
                                            <p className="text-sm mt-1">Try adjusting filters or create a new quote to get started.</p>
                                            <Link
                                                href="/sales/estimates/new"
                                ...
                                estimates?.data?.map((estimate: Estimate) => (
                                    <tr key={estimate.id} className="hover:bg-slate-50 transition-colors group">
                                        <td className="px-6 py-4">
                                            <Link href={`/sales/estimates/${estimate.id}`} className="block">
                                                ...
                                                <div className="text-sm text-slate-500 mt-0.5">
                                                    ...
                                                    {estimate.expiry_date && (
                                                        <span className="ml-2 text-slate-400">
                                                            (Expires: {format(new Date(estimate.expiry_date), 'dd MMM')})
                                                        </span>
                                                    )}
                                                </div>
                                            </Link>
                                        ...
                                        <td className="px-6 py-4">
                                            <Link href={`/masters/parties/${estimate.party.id}`} className="font-medium text-slate-700 hover:text-blue-600 transition-colors">
                                                {estimate.party.name}
                                            </Link>
                                        </td>
                                        <td className="px-6 py-4">
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <div className="font-semibold text-slate-800">
                                                ₹{estimate.total.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                                            </div>
                                        </td>
                ...
                <div className="p-4 border-t border-slate-200 flex items-center justify-between text-sm text-slate-500">
                    <div>Showing {estimates?.data?.length || 0} estimates</div>
                    {estimates?.last_page > 1 && (
                        <div className="flex gap-1">
                            <span>Page {estimates?.current_page} of {estimates?.last_page}</span>
                        </div>
                    )}
                </div>
            </div>
