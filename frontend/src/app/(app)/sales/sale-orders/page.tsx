...
export default function SaleOrdersPage() {
    ...
    const { data: orders, isLoading } = useQuery({
        queryKey: ['sale_orders', search, statusFilter],
        queryFn: async () => {
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-slate-500">Track and manage customer orders before invoicing</p>
                </div>

                <Link
                    href="/sales/sale-orders/new"
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-slate-200">
                            {isLoading ? (
                                ...
                                <tr>
                                    <td colSpan={5} className="px-6 py-12 text-center text-slate-500">
                                        <div className="flex flex-col items-center justify-center">
                                            ...
                                            <p className="text-lg font-medium text-slate-700">No sale orders found</p>
                                            <Link
                                                href="/sales/sale-orders/new"
                                ...
                                orders?.data?.map((order: SaleOrder) => (
                                    <tr key={order.id} className="hover:bg-slate-50 transition-colors group">
                                        <td className="px-6 py-4">
                                            <Link href={`/sales/sale-orders/${order.id}`} className="block">
                                        ...
                                        <td className="px-6 py-4">
                                            <Link href={`/masters/parties/${order.party.id}`} className="font-medium text-slate-700 hover:text-blue-600 transition-colors">
                                                {order.party.name}
                                            </Link>
                                        </td>
                                        <td className="px-6 py-4">
