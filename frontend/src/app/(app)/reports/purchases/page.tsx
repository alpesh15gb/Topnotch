    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Purchase performance overview</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: 'Total Purchases', value: `₹${(data?.total_purchases ?? 0).toLocaleString('en-IN')}` },
            { label: 'Total Bills', value: data?.bill_count ?? 0 },
            { label: 'Total Tax Paid', value: `₹${(data?.total_tax ?? 0).toLocaleString('en-IN')}` },
          ].map(card => (
