...
export default function PartiesScreen() {
  ...
  const { data, isLoading, isError, refetch, isRefetching } = useQuery({
    ...
    queryFn: () => api.get('/v1/parties', { params: { search: search || undefined } }).then((r) => r.data.data || []),
  });

  const parties: any[] = data || [];
  ...
  return (
    <View style={s.container}>
      ...
      {isLoading ? (
        ...
        <View style={s.center}>
          <Text style={{ color: '#F87171' }}>Failed to load parties</Text>
        </View>
      ) : parties.length === 0 ? (
        <ScrollView style={{ flex: 1 }} refreshControl={<RefreshControl refreshing={isRefetching} onRefresh={refetch} tintColor={C.accent} />}>
          <View style={s.empty}>
            <View style={s.emptyIcon}><Users color="#8892A4" size={32} /></View>
            <Text style={s.emptyTitle}>No parties added yet</Text>
            <Text style={s.emptySub}>Add customers and suppliers to get started</Text>
          </View>
        </ScrollView>
      ) : (
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity style={s.card} onPress={() => router.push(`/parties/${item.id}` as any)} activeOpacity={0.75}>
              <View style={s.cardTop}>
                <Text style={s.name}>{item.name}</Text>
                <View style={[s.badge, { backgroundColor: item.type === 'customer' ? '#1E3A5F' : '#334155' }]}>
                  <Text style={[s.badgeText, { color: item.type === 'customer' ? '#60A5FA' : '#94A3B8' }]}>
                    {item.type.charAt(0).toUpperCase() + item.type.slice(1)}
                  </Text>
                </View>
              </View>
              <View style={s.cardBottom}>
                <Text style={s.contact}>{item.email || item.phone || 'No contact info'}</Text>
                <Text style={s.balance}>₹{(item.current_balance || 0).toLocaleString('en-IN')}</Text>
              </View>
            </TouchableOpacity>
          )}
        />
      ...
      <TouchableOpacity style={s.fab} onPress={() => router.push('/parties/new' as any)}>
...
const s = StyleSheet.create({
  ...
  contact: { color: C.sub, fontSize: 13, flex: 1 },
  balance: { color: C.text, fontWeight: '700', fontSize: 14 },
});
